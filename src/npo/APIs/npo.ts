...
export class NpoAPI {
    static async getAllNpos (page: number = 1, limit: number = 30) {
        try {
            const token = Cookies.get('token');
            const res = await fetch(`${baseURL}/np-orders/?page=${page}&limit=${limit}`, { 
    ...
    static async getOrderItems(id: string){
        try {
            const token = Cookies.get('token');
            const res = await fetch(`${baseURL}/np-orders/one/items/${id}`, { 
                method : 'GET', credentials : 'include', headers : { 
                    'Content-Type': 'application/json', 
                    "authorization": `Bearer ${token}`
                } 
